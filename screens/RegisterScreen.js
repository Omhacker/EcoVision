...
export default function RegisterScreen({ navigation }) {
  ...
  const handleRegister = async () => {
    try {
      ...
      // Store user info with role in Firestore
      await setDoc(doc(db, "users", user.uid), {
        name,
        phone,
        email,
        role, // Store user role (Admin or Driver)
        uid: user.uid,
      });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.formContainer}>
        ...
        <View style={styles.roleContainer}>
          <TouchableOpacity onPress={() => setRole("Driver")} style={[styles.roleButton, role === "Driver" && styles.selectedRole]}>
...
const styles = StyleSheet.create({
  ...
  roleButton: { padding: 10, borderWidth: 1, borderRadius: 5, marginHorizontal: 10 },
  selectedRole: { backgroundColor: "#43A047" },
